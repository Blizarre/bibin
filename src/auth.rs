...
pub struct AuthKeyHash(PasswordHashString);

impl<'de> serde::Deserialize<'de> for AuthKeyHash {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl Visitor<'_> for HashVisitor {
            ...
            where
                E: serde::de::Error,
            {
                let password_hash = match PasswordHashString::new(v) {
                            Err(error) => return Err(serde::de::Error::custom(format!("The password hash is invalid. It needs to be generated by bibin_pwd_hash. Error: {}", error))),
                            Ok(hash) => hash,
                        };
                Ok(password_hash)
            }
            ...
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                formatter.write_str("Password hash")
            }
        }
        let pwd_hash = deserializer.deserialize_str(HashVisitor)?;
        Ok(AuthKeyHash(pwd_hash))
    }
}
...
impl AuthKey {
    pub fn is_valid(&self, hash: &AuthKeyHash) -> bool {
        Argon2::default()
            .verify_password(self.0.as_bytes(), &hash.0.password_hash())
            .is_ok()
    }
}
